...
This module extends the existing Flask app without replacing the working
Agent-AI-coach routes. It is intentionally focused on sales activity:
- daily WhatsApp nurture via the existing Interakt integration
- lead capture
- PBPartners quotation hand-off
- quotation screenshot -> AI customer/agent message
- new-agent lead capture
- compact sales dashboard

Run with: gunicorn fast_launch:app
"""
...
from app import app, AGENTS, first_name, send_whatsapp_template, send_text_message
...
def _agent_from_payload(data):
    aid = str(data.get("agent_id", "")).strip()
    if aid:
        return next((a for a in AGENTS if a.get("agent_id") == aid), None)
    phone = _phone(data.get("phone"))
    if phone:
        return next((a for a in AGENTS if _phone(a.get("phone")) == phone), None)
    return None


def _ai_message(quote_text, customer_name="Customer", language="marathi"):
    if not OPENAI_API_KEY:
        return _fallback_message(quote_text, customer_name, language)

    prompt = f"""Create a short insurance quotation follow-up message.
Language: {language}. Customer: {customer_name}.
The quotation was generated on PBPartners.com by the agent. Do not invent
premium, coverage, insurer, IDV or payment details. Use only the quotation
text supplied below. Make it WhatsApp-ready, polite and sales-focused.
Do not claim that Priya generated or issued the policy. End with one clear
call-to-action.
...
def _fallback_message(quote_text, customer_name, language):
...
def capture_lead():
    ...
    # Keep a lightweight runtime queue; production persistence can be moved to
    # PostgreSQL without changing the API contract.
...
def quote_message_api():
    ...
    return jsonify({"ok":True, "message":message, "pbpartners_url":PBPARTNERS_URL})
...
def daily_nurture():
    ...
    Use an external cron to call this endpoint repeatedly (for example,
    batches of 25). This avoids an in-process scheduler being lost on restart.
    """
    if not _authorized():
        return jsonify({"error":"Unauthorized"}), 401
    ...
    # The source of truth can be a Google Sheet through the existing endpoint.
    try:
    ...
    results = []
    for agent in batch:
        lang = agent.get("language", language) or language
        # Only approved Interakt templates should be used for proactive/outside-24h messaging.
        result = send_whatsapp_template(agent["name"], agent["phone"], lang)
...
def sales_dashboard():
    ...
    return f'''<!doctype html><html lang="en"><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>PartnersHub AI — Sales Command</title><style>body{{font-family:Arial;background:#07111f;color:#fff;max-width:900px;margin:auto;padding:18px}}.grid{{display:grid;grid-template-columns:repeat(2,1fr);gap:12px}}.card{{background:#111d2e;border:1px solid #26364d;border-radius:16px;padding:18px}}.big{{font-size:30px;font-weight:800}}.btn{{display:inline-block;padding:12px 16px;border-radius:10px;background:#ff6b1a;color:#fff;text-decoration:none;margin:5px 5px 0 0}}small{{color:#9fb0c4}}</style></head><body><h1>🤖 Priya AI — Sales Command</h1><p><small>Daily business target: ₹{target:,}</small></p><div class="grid"><div class="card"><small>Agents loaded</small><div class="big">{len(AGENTS)}</div></div><div class="card"><small>PBPartners quotation</small><div class="big">LIVE LINK</div></div><div class="card"><small>Daily nurture</small><div class="big">25 / batch</div></div><div class="card"><small>New agent leads</small><div class="big">ACTIVE</div></div></div><p><a class="btn" href="/pbpartners">Open PBPartners</a><a class="btn" href="/agents">Agent Panel</a><a class="btn" href="/marketing-page">Find New Agents</a></p><p><small>Quotation is generated on PBPartners. Priya AI only prepares the follow-up/customer message from the quotation data supplied by the agent.</small></p></body></html>'''


# Preserve Flask's existing entry point when this module is used directly.
application = app
