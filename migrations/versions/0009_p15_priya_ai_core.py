...
def upgrade():
    op.create_table("ai_skills",
        sa.Column("id", sa.String(36), primary_key=True), sa.Column("code", sa.String(80), nullable=False),
        sa.Column("name", sa.String(180), nullable=False), sa.Column("category", sa.String(50), nullable=False),
    ...
    op.create_table("ai_knowledge_sources",
        ...
        sa.Column("source_type", sa.String(50), nullable=False, server_default="internal"), sa.Column("source_uri", sa.String(700)),
        sa.Column("content", sa.Text(), nullable=False), sa.Column("status", sa.String(30), nullable=False, server_default="active"),
    ...
    op.create_table("ai_tasks",
        ...
        sa.Column("rm_id", sa.String(36), sa.ForeignKey("rms.id", ondelete="SET NULL")), sa.Column("lead_id", sa.String(36), sa.ForeignKey("leads.id", ondelete="SET NULL")),
        sa.Column("policy_id", sa.String(36), sa.ForeignKey("policies.id", ondelete="SET NULL")), sa.Column("input_json", sa.Text()),
        sa.Column("output_json", sa.Text()), sa.Column("status", sa.String(30), nullable=False, server_default="queued"),
        sa.Column("created_by", sa.String(36), sa.ForeignKey("users.id", ondelete="SET NULL")), sa.Column("created_at", sa.DateTime(timezone=True), nullable=False),
        sa.Column("completed_at", sa.DateTime(timezone=True)))
    for name, cols in {"ix_ai_tasks_skill_code":["skill_code"],"ix_ai_tasks_agent_id":["agent_id"],"ix_ai_tasks_rm_id":["rm_id"],"ix_ai_tasks_lead_id":["lead_id"],"ix_ai_tasks_policy_id":["policy_id"],"ix_ai_tasks_status":["status"]}.items():
        op.create_index(name, "ai_tasks", cols)
    ...
    op.create_table("ai_recommendations",
        ...
        sa.Column("rm_id", sa.String(36), sa.ForeignKey("rms.id", ondelete="CASCADE")), sa.Column("priority", sa.Integer(), nullable=False, server_default="50"),
        sa.Column("action_type", sa.String(60), nullable=False), sa.Column("reason", sa.Text(), nullable=False), sa.Column("suggested_message", sa.Text()),
        sa.Column("source_ids_json", sa.Text()), sa.Column("status", sa.String(30), nullable=False, server_default="open"), sa.Column("outcome", sa.Text()),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False), sa.Column("completed_at", sa.DateTime(timezone=True)))
    for name, cols in {"ix_ai_recommendations_scope":["scope"],"ix_ai_recommendations_skill_code":["skill_code"],"ix_ai_recommendations_agent_id":["agent_id"],"ix_ai_recommendations_rm_id":["rm_id"],"ix_ai_recommendations_status":["status"]}.items():
        op.create_index(name, "ai_recommendations", cols)
